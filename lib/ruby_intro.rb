...
#most often in ruby we to do it by useing code -1.1
#product gives a cartesian product. Once we get that we can use any? to check all the cartesian pairs which sum up to n. 
#This approach will fail for cases where array might have duplicate.
#for example let arr=[1,2,4,4] and n be 8, 1.1  will fail, since it will eliminate 4,4 duplet because of c[0] != c[1].
...
def sum_to_n? arr, n
  #arr.product(arr).any? {|c| sum(c) == n && c[0] != c[1] }    ----1.1
  arr = arr.sort
  low = 0
  high = arr.length - 1
  while low < high
